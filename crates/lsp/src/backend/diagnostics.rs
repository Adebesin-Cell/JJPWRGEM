...
use crate::{backend::SOURCE, range::span_to_range};
pub fn diagnostics_from_error(uri: &Uri, text: &str, e: &jjpwrgem_parse::Error) -> Vec<Diagnostic> {
    let related: Vec<_> = Vec::<Context<'_>>::from(e)
        .into_iter()
        .map(|ctx| DiagnosticRelatedInformation {
            location: Location {
                uri: uri.clone(),
                range: span_to_range(text, ctx.span),
            },
    ...
    vec![Diagnostic {
        range: span_to_range(text, *e.range()),
...
pub fn build_code_actions(
    ...
    let diagnostics = Some(diagnostics_from_error(uri, text, err));
    Vec::<Patch<'_>>::from(err)
        ...
        .map(|patch| {
            let changes = HashMap::from([(
                uri.clone(),
                vec![TextEdit {
                    range: span_to_range(text, patch.span),
